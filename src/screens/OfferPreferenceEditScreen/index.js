...
const OfferPreferenceEditScreen = ({navigation}) => {
  ...
  const [canEdit, ] = useState(false); // TODO: integrate this once api ready
  ...
  const {data: userMembershipData, loading: userMembershipLoading} = useQuery(GET_USER_MEMBERSHIP_API, apiContext);
  ...
  const handleEditPress = () => {
    if(canEdit)
      navigation.navigate('offers_select', {onGoBack: handleOnGoBack});
    else
      setShowModal(true)
  };
